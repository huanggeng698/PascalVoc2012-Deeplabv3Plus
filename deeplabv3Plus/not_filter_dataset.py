...
def img_transforms(data, label, high, width,colormap):
    ...
    if data.size[0] < width:
        n_width = data.size[0]
        # diff = width - n_width
        # data = Ft.pad(data, [diff//2, 0, diff-diff//2, 0])
        # label = Ft.pad(label, [diff//2, 0, diff-diff//2, 0])
        data = Ft.pad(data, padding=int((1 + width-n_width) / 2))
        label = Ft.pad(label, padding=int((1 + width-n_width) / 2))
    ...
    if data.size[1] < high:
        ...
        data = Ft.pad(data, padding=int((1 + high-n_high) / 2))
        label = Ft.pad(label, padding=int((1 + high-n_high) / 2))
    # im_width, im_high = data.size
    if data.size[0] > width or data.size[1] > high:
